...
def _get_ellipse_datamodel(sbthresh, bands=['g', 'r', 'z']):
    ...
    for band in bands:
        cols.append(
            ('{}_sma'.format(band), u.pixel),
            ('{}_intens'.format(band), u.maggy/u.arcsec**2),
            ('{}_intens_err'.format(band), u.maggy/u.arcsec**2),
            ('{}_eps'.format(band), ''),
            ('{}_eps_err'.format(band), ''),
            ('{}_pa'.format(band), u.degree),
            ('{}_pa_err'.format(band), u.degree),
            ('{}_x0'.format(band), u.pixel),
            ('{}_x0_err'.format(band), u.pixel),
            ('{}_y0'.format(band), u.pixel),
            ('{}_y0_err'.format(band), u.pixel),
            ('{}_a3'.format(band), ''), # units?
            ('{}_a3_err'.format(band), ''),
            ('{}_a4'.format(band), ''),
            ('{}_a4_err'.format(band), ''),
            ('{}_rms'.format(band), u.maggy/u.arcsec**2),
            ('{}_pix_stddev'.format(band), u.maggy/u.arcsec**2),
            ('{}_stop_code'.format(band), ''),
            ('{}_ndata'.format(band), ''),
            ('{}_nflag'.format(band), ''),
            ('{}_niter'.format(band), '')
            ('{}_cog_sma'.format(band), u.arcsec),
            ('{}_cog_mag'.format(band), u.mag),
            ('{}_cog_magerr'.format(band), u.mag),
            ('{}_cog_params_mtot'.format(band), u.mag),
            ('{}_cog_params_m0'.format(band), u.mag),
            ('{}_cog_params_alpha1'.format(band), ''),
            ('{}_cog_params_alpha2'.format(band), ''),
            ('{}_cog_params_chi2'.format(band), ''),
            )
    ...
    for thresh in sbthresh:
        cols.append(
            ('radius_sb{:0g}'.format(thresh), u.arcsec)
            ('radius_sb{:0g}_err'.format(thresh), u.arcsec)
            )
    for band in bands:
        for thresh in sbthresh:
            cols.append(
                ('{}_mag_sb{:0g}'.format(band, thresh), u.mag),
                ('{}_mag_sb{:0g}_err'.format(band, thresh), u.mag),
                )
