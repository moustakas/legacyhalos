...
#srun -n 8 -c 32 shifter --module=mpich-cle6 $LEGACYHALOS_CODE_DIR/bin/SGA/SGA-customredux-mpi.sh coadds 8 > coadds-customredux.log.1 2>&1 &
...
#source /global/u2/i/ioannis/repos/git/legacyhalos/bin/SGA/SGA-customredux-env

if [ $stage = "test" ]; then
    time python $LEGACYHALOS_CODE_DIR/bin/SGA/SGA-mpi --help
elif [ $stage = "coadds" ]; then
    time python $LEGACYHALOS_CODE_DIR/bin/SGA/SGA-mpi --coadds --nproc $ncores --mpi --customredux --verbose --just-coadds
