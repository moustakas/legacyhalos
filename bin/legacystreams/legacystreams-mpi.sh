#! /bin/bash

# Shell script for running the various stages of the legacyhalos code using
# MPI+shifter at NERSC. Required arguments:
#   {1} stage [coadds, pipeline-coadds, ellipse, htmlplots]
#   {2} ncores [should match the resources requested.]

# Example: build the coadds using 16 MPI tasks with 8 cores per node (and therefore 16*8/32=4 nodes)

#salloc -N 4 -C haswell -A desi -L cfs,SCRATCH -t 04:00:00 --qos interactive --image=legacysurvey/legacyhalos:v0.0.4
#srun -n 4 -c 32 shifter --module=mpich-cle6 $LEGACYHALOS_CODE_DIR/bin/legacystreams/legacystreams-mpi.sh coadds 32 > coadds.log.1 2>&1 &
#srun -n 64 -c 4 shifter --module=mpich-cle6 $LEGACYHALOS_CODE_DIR/bin/legacystreams/legacystreams-mpi.sh ellipse 4 > ellipse.log.1 2>&1 &
#srun -n 64 -c 1 shifter --module=mpich-cle6 $LEGACYHALOS_CODE_DIR/bin/legacystreams/legacystreams-mpi.sh htmlplots 1 > htmlplots.log.1 2>&1 &

# Grab the input arguments--
stage=$1
ncores=$2

project=legacystreams

source $LEGACYHALOS_CODE_DIR/bin/${project}/${project}-env

if [ $stage = "test" ]; then
    time python $LEGACYHALOS_CODE_DIR/bin/${project}/${project}-mpi --help
elif [ $stage = "coadds" ]; then
    time python $LEGACYHALOS_CODE_DIR/bin/${project}/${project}-mpi --coadds --nproc $ncores --mpi --verbose --no-cleanup --clobber --galaxylist NGC0259 NGC2460 NGC3521 NGC3631 NGC4684
elif [ $stage = "pipeline-coadds" ]; then
    time python $LEGACYHALOS_CODE_DIR/bin/${project}/${project}-mpi --pipeline-coadds --nproc $ncores --mpi --verbose
elif [ $stage = "ellipse" ]; then
    time python $LEGACYHALOS_CODE_DIR/bin/${project}/${project}-mpi --ellipse --nproc $ncores --mpi --verbose
elif [ $stage = "htmlplots" ]; then
    time python $LEGACYHALOS_CODE_DIR/bin/${project}/${project}-mpi --htmlplots --nproc $ncores --mpi --verbose
else
    echo "Unrecognized stage "$stage
fi
